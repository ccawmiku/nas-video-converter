...
- FFprobe 或严格完整解码确认损坏、且检测期间未变化的源文件，会通过同文件系统安全改名移动到 `损坏文件`；保留相对目录，不覆盖同名文件。权限或普通读取失败不会触发移动。
...
|---|---|
| 无需转换 | 已是 MP4，只做完整性检测 |
| 可无损换封装 | 非 MP4，兼容轨道原样写入 MP4；仅 `data/timed_id3` 定时数据轨道会被明确排除 |
| 需要重新编码 | 普通 SDR 8-bit 视频转 H.264；Opus 音轨单独转 AAC；其他兼容轨道原样复制 |
...
FFmpeg 操作使用 `-progress pipe:1` 的真实输出展示阶段、文件/整体百分比、数量、体积、已用时间、ETA、speed、输出时间和总时长；后端每 0.2 秒检查并通过 SSE 转发新进度。首次连接只订阅当前之后的新事件，历史完成或取消任务不会被重新显示为实时任务。首页只保留统计、实时任务、设置和功能入口；文件计划按每页 100 条显示，任务历史、系统日志、转换记录和中断文件分别在独立页面按需加载。网页顶部和实时任务区域会显示当前实际使用的编码后端（Intel QSV、Intel VAAPI、libx264 或流复制）。暂停/继续在 Linux 上使用进程信号；取消会终止 FFmpeg 并保存已产生的临时输出。
