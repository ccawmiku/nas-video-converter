...
参见 [docs/synology.md](docs/synology.md)。容器只发布 `linux/amd64`，不支持 ARM 群晖。默认进程身份为 `1026:100`，可通过 `PUID`/`PGID` 覆盖。数据库和设置位于 `/config/nas-video-converter.db`。N95 等 Intel 核显可通过 `docker-compose.intel-qsv.yml` 映射 `/dev/dri/renderD128`；网页默认自动检测 QSV，不可用时回退到软件编码。
