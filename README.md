...
转码保留分辨率和帧率，使用 `yuv420p`、preset medium，不设置 maxrate 和输出大小上限。三档为 CRF 16 / 18（默认）/ 20。输出变大仍完成并显示黄色警告，不自动再转第二次。
...
参见 [docs/synology.md](docs/synology.md)。容器只发布 `linux/amd64`，不支持 ARM 群晖。默认进程身份为 `1024:100`，可通过 `PUID`/`PGID` 覆盖。数据库和设置位于 `/config/nas-video-converter.db`。
